...
%ontains recursors for primitive recursion of type level $\le n.$  \\ 
...
\\[1mm] 
%These results complement the ones 
...
So far the aforementioned elimination method (see \cite{Kohlenbach98,Kohlenbach00}) has not been 
...
\cite{Kohlenbach(weakcompactness)} for discussions).  
...
\subsection{Interpreting Bolzano-Weierstra{\ss}} \label{ss:bw}
...
\subsubsection*{A Simple Proof of $\BW$ based on $\SiLm\WKL$} \label{ss:spuWKL}
...
\subsubsection*{Functional Interpretation of $\BW^\omega_\RR$} \label{ss:fafi}
