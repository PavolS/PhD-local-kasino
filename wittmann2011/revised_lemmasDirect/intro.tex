...
\]
then the sequence of the Ces{\`a}ro means converges in norm to a fixed point in norm. Shortly after this,
...
\]
for some sequence $(\alpha_n)$ of nonnegative real numbers which converges to $1$. 
...
\]
and analogously an odd and asymptotically nonexpansive mapping satisfies the asymptotic version
\[
\forall n\in\NN\ \forall u,v\in C\ (\| T^nu + T^nv \| \leq \alpha_n\|u + v\|), \tag{$W^-$}\label{e:W-assym}
\]
for some sequence $(\alpha_n)$ of nonnegative real numbers which converges to $1$. In 1990, Wittmann~\cite{Wittmann90}
proved a generalization of Baillon's strong convergence theorem 
to an arbitrary $C$ and a mapping satisfying the condition~\eqref{e:W-assym} (see also Theorem 2.2 in~\cite{Wittmann90} and Theorem~\ref{t:22} below).
...
\label{f:METtree}
\end{figure}
\footnotetext{While the results were essentially available on \url{http://arxive.org/} since 2007, the paper as such was submitted in 2008. Thereafter Kohlenbach and Leu{\c{s}}tean 
...
be a mapping satisfying 
\[
\forall x,y\in S\ (\| T^nx + T^ny \| \leq \alpha_n\|x + y\|),\quad
...
\end{align*}
for a primitive recursive $M$, where $K$ is a rate of convergence for the sequence $(\alpha_n)$ in
...
this bound depends only on the bound $b$ and not on $S$, $T$ or $x$.
...
see this quickly on the term $M$ in Proposition~\ref{p:fin} below. While $F$ as a function is defined via iteration of the counterfunction
...
\paragraph{Outline} We discuss the application of general logical metatheorems in more detail in Section~\ref{s:meta} which is not
necessary to understand and verify our main results. We present these, namely the explicit bounds
for all three theorems in Wittmann's paper~\cite{Wittmann90}, in Section~\ref{s:main}. Section~\ref{s:proof} contains some
lemmas necessary to prove the core proposition (Proposition~\ref{p:fin}) for the main results and the actual proof of this proposition.
