...
In Proposition \ref{p:nonLearnablePhi} 
...
\begin{dfn}[(B,L)-learnability for general (not necessarily monotone) formulas]\label{d:fmcNum}
...
(see~\cite{KS13}). \\ [2mm]
%One can also modify Toftdal's proof so that only the $\Pi^0_1$-FAC 
...
from \cite{Kohlenbach(lowrate)} (see also \cite{Kohlenbach08}).
